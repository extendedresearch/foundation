...
fn ships_and_matches(name: &str) {
    ...
    let copy = read(&crate_dir.join(name)).unwrap_or_else(|| {
        panic!("crates/status/{name} is missing, so the published crate would ship without it")
    });
    ...
    if let Some(root) = read(&crate_dir.join("../..").join(name)) {
        assert_eq!(
            copy, root,
            "crates/status/{name} differs from the repository's {name}; copy the root file over it"
        );
    }
}
